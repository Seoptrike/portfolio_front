...
export default function AuthProvider({ children }) {
    ...
    // 로그인 상태는 앱 시작 시 한 번만 확인 (로그아웃 시 loginCheckHandler로 갱신)
    useEffect(() => {
        loginCheckHandler();
    }, []);

    // 호스트 권한은 서버 재호출 없이 현재 경로에서 파생
    const urlUsername = extractUsernameFromPath(location.pathname);
    const isHost = Boolean(isAdmin || (loginName && urlUsername === loginName));
